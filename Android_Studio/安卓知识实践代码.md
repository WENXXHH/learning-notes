...
### 9, PlaceViewModel
    ...
    class PlaceViewModel : ViewModel() {
        ...
        // 关键修改：处理 Result 包装类型
        val placeLiveData = searchLiveData.switchMap<String, List<Place>> { query ->
            // 将 Repository 返回的 LiveData<Result<List<Place>>> 转换为 LiveData<List<Place>>
            Repository.searchPlaces(query).map { result ->
...
### 2, 定义RecyclerView的子项布局
    在layout目录下新建一个place_item.xml文件，
    一个用于显示搜索到的地区名，一个用于显示该地区的详细地址。
    代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    <com.google.android.material.card.MaterialCardView 
     xmlns:android="http://schemas.android.com/apk/res/android" 
...
### 3, 为RecyclerView准备适配器
    在ui/place包下新建一个PlaceAdapter类，让这个适配器继承自RecyclerView.Adapter，
    并将泛型指定为PlaceAdapter.ViewHolder，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    class PlaceAdapter(private val fragment: Fragment, private val placeList: List<Place>) :
        ...
        //RecyclerView 首次创建列表项时调用，返回封装了视图的ViewHolder对象
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        //RecyclerView 需要显示/更新某个位置时调用
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            val place = placeList[position]  // 获取当前位置的数据
            holder.placeName.text = place.name       // 设置名称
            holder.placeAddress.text = place.address // 设置地址
        }
...
### 1，RealtimeResponse
    在logic/model包下新建一个RealtimeResponse.kt文件，并在这个文件中编写如下代码：
    ——————————————————————————————————————————————————————————————————————
...
### 2，DailyResponse
    同样在logic/model包下新建一个DailyResponse.kt文件，并编写如下代码：
    ——————————————————————————————————————————————————————————————————————
...
### 3，Weather
    在logic/model包下再定义一个Weather类，用于将Realtime和Daily对象封装起来，
    代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    data class Weather(val realtime: 
    RealtimeResponse.Realtime, 
    val daily: DailyResponse.Daily)
    ——————————————————————————————————————————————————————————————————————

### 4，WeatherService
    现在定义一个用于访问天气信息API的Retrofit接口，
    在logic/network包下新建WeatherService接口，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
...
### 5，SunnyWeatherNetwork
    接下来我们需要在SunnyWeatherNetwork这个网络数据源访问入口对新增的
    WeatherService接口进行封装。修改SunnyWeatherNetwork中的代码，如下所示：
    ——————————————————————————————————————————————————————————————————————
    object SunnyWeatherNetwork { 
...
### 7，修改Repository
    修改Repository成只要进行一次try catch处理，代码如下：
    ——————————————————————————————————————————————————————————————————————
    object Repository { 
...
### 8，WeatherViewModel
    定义ViewModel层。在ui/weather包下新建一个WeatherViewModel，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    ...
    // 使用 switchMap 扩展函数替代 Transformations.switchMap
...
### 1，now.xml
    首先创建一个用于显示天气信息的Activity。右击ui/weather包→New→Activity→Empty
    Activity，创建一个WeatherActivity，并将布局名指定成activity_weather.xml。
    右击res/layout→New→Layout resource ﬁle，新建一个now.xml作为当前天气信息的布局，
    代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    <RelativeLayout xmlns:android="http://schemas.android.com/apk/res/android" 
...
### 2，forecast.xml
    ...
    <com.google.android.material.card.MaterialCardView 
     xmlns:android="http://schemas.android.com/apk/res/android" 
     xmlns:app="http://schemas.android.com/apk/res-auto" www.blogss.cn
     android:layout_width="match_parent" 
     android:layout_height="wrap_content" 
     android:layout_marginLeft="15dp" 
     android:layout_marginRight="15dp" 
...
### 3，forecast_item.xml
    再定义一个未来天气信息的子项布局，创建forecast_item.xml文件，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    <LinearLayout xmlns:android="http://schemas.android.com/apk/res/android" 
     android:layout_width="match_parent" 
     android:layout_height="wrap_content" 
...
### 5，activity_weather.xml
    接下来的工作就是将它们引入activity_weather.xml中，如下所示：
    ——————————————————————————————————————————————————————————————————————
    <ScrollView 
     xmlns:android="http://schemas.android.com/apk/res/android" 
...
### 6，Sky.kt
    在logic/model包下新建一个Sky.kt文件，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
...
### 7，WeatherActivity
    ...
    class WeatherActivity : AppCompatActivity() {
        ...
        private fun showWeatherInfo(weather: Weather) {
            ...
            val forecastLayout = binding.root.findViewById<android.widget.LinearLayout>(R.id.forecastLayout)
            forecastLayout.removeAllViews()
            val days = daily.skycon.size
            for (i in 0 until days) {
                val skycon = daily.skycon[i]
                val temperature = daily.temperature[i]
                val itemView = ForecastItemBinding.inflate(
                    LayoutInflater.from(this),
                    forecastLayout,
                    false
                )
                val simpleDateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
                itemView.dateInfo.text = simpleDateFormat.format(skycon.date)
                val sky = getSky(skycon.value)
                itemView.skyIcon.setImageResource(sky.icon)
                itemView.skyInfo.text = sky.info
                val tempText = "${temperature.min.toInt()} ~ ${temperature.max.toInt()} ℃"
                itemView.temperatureInfo.text = tempText
                forecastLayout.addView(itemView.root)
            }
            // 填充life_index.xml布局中的数据
            val lifeIndex = daily.lifeIndex
            binding.root.findViewById<android.widget.TextView>(R.id.coldRiskText).text = lifeIndex.coldRisk[0].desc
            binding.root.findViewById<android.widget.TextView>(R.id.dressingText).text = lifeIndex.dressing[0].desc
            binding.root.findViewById<android.widget.TextView>(R.id.ultravioletText).text = lifeIndex.ultraviolet[0].desc
            binding.root.findViewById<android.widget.TextView>(R.id.carWashingText).text = lifeIndex.carWashing[0].desc
...
### 10，修改now.xml
    ...
    <RelativeLayout xmlns:android="http://schemas.android.com/apk/res/android" 
     ...
     android:layout_width="match_parent" 
     android:layout_height="70dp" 
     android:fitsSystemWindows="true"> //添加这一行
     ... 
     </FrameLayout> 
     ... 
    </RelativeLayout>
...
### 1，PlaceDao
    首先在logic/dao包下新建一个PlaceDao单例类，并编写如下代码：
    ——————————————————————————————————————————————————————————————————————
    object PlaceDao { 
...
### 3，修改PlaceViewModel
    在PlaceViewModel中再进行一层封装才行，代码如下所示：
    ——————————————————————————————————————————————————————————————————————
    class PlaceViewModel : ViewModel() { 
     ... 
...
### 3，切换城市，修改now.xml
    ...
    <RelativeLayout xmlns:android="http://schemas.android.com/apk/res/android" 
     ...
     android:contentDescription="@string/btn_home_desc"/> 
     ... 
     </FrameLayout> 
     ... 
    </RelativeLayout> 
